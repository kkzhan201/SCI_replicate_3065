...
parser.add_argument('--epochs', type=int, default=100, help='epochs')#if use 1000
parser.add_argument('--lr', type=float, default=0.0009, help='learning rate') #then adjust to 0.0003
...
if torch.cuda.is_available():
    if args.cuda:
        # torch.set_default_tensor_type('torch.cuda.FloatTensor')# This cannot be used in pytorch 2.x
    ...
    if not args.cuda:
        ...
        # torch.set_default_tensor_type('torch.FloatTensor') # This cannot be used in pytorch 2.x
        torch.set_default_dtype(torch.float)
...
def main():
    ...
    train_low_data_names = './newlow' # Here defines the directory of the train set
    TrainDataset = MemoryFriendlyLoader(img_dir=train_low_data_names, task='train') # trainA


    test_low_data_names = './data/medium' # Here defines the directory of the val set
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (input, _) in enumerate(train_queue):
            total_step += 1
            # input = Variable(input, requires_grad=False).cuda() # This cannot be used in pytorch 2.x
            ...
            print(f"Time taken to process image {batch_idx}: {time_taken} seconds")
        ...
        if epoch % 1 == 0 and total_step != 0:
            ...
            with torch.no_grad():
                for _, (input, image_name) in enumerate(test_queue):
                    # input = Variable(input, volatile=True).cuda() # no longer support in Torch 2.x
                    input = input.to(device)
